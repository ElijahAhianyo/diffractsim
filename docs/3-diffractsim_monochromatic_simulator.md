# Diffractsim Monochromatic Simulator
...
<!---Documatic-section-diffractsim.monochromatic_simulator.MonochromaticField-start--->
## diffractsim.monochromatic_simulator.MonochromaticField

<!---Documatic-section-MonochromaticField-start--->
<!---Documatic-block-diffractsim.monochromatic_simulator.MonochromaticField-start--->
<details>
	<summary><code>diffractsim.monochromatic_simulator.MonochromaticField</code> code snippet</summary>
...
class MonochromaticField:
    ...
    def propagate_to_lens_focal_plane(self, focal_length, x_interval, y_interval):
        ...
        C = bluestein_fft2(self.E, x_interval[0] / (focal_length * self.λ), x_interval[1] / (focal_length * self.λ), 1 / self.dx, y_interval[0] / (focal_length * self.λ), y_interval[1] / (focal_length * self.λ), 1 / self.dy)
    ...
    def get_colors(self):
        ...
        rgb = self.cs.wavelength_to_sRGB(self.λ / nm, 10 * I.ravel()).T.reshape((self.Ny, self.Nx, 3))
        return rgb
    ...
    def interpolate(self, Nx, Ny):
        ...
        if bd != np:
            self.E = self.E.get()
        fun_real = interp2d(self.dx * (np.arange(self.Nx) - self.Nx // 2), self.dy * (np.arange(self.Ny) - self.Ny // 2), np.real(self.E), kind='cubic')
        fun_imag = interp2d(self.dx * (np.arange(self.Nx) - self.Nx // 2), self.dy * (np.arange(self.Ny) - self.Ny // 2), np.imag(self.E), kind='cubic')
        self.Nx = Nx
        self.Ny = Ny
    ...
    def __add__(self, Field):
        ...
        if self.extent_x == Field.extent_x and self.extent_y == Field.extent_y and (self.Nx == Field.Nx) and (self.Ny == Field.Ny) and (self.λ == Field.λ):
            ...
            raise ValueError('The wavelength, dimensions and sampling of the interfering fields must be identical')
...
<!---Documatic-block-diffractsim.monochromatic_simulator.MonochromaticField-end--->
<!---Documatic-section-MonochromaticField-end--->

# #
<!---Documatic-section-diffractsim.monochromatic_simulator.MonochromaticField-end--->
